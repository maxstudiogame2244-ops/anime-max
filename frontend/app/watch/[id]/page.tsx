...
import {
  ...
  getHiAnimeStreamingLink,
  getToonStreamStreamingLink,
...
export async function generateMetadata({
  ...
  if (mediaInfo.format == "MOVIE") {
    pageTitle = `Watch ${mediaInfo.title.userPreferred} | Anime Max`;
  } else {
    // ACTES AS DEFAULT VALUE FOR PAGE PROPS
    if (Object.keys(searchParams).length === 0) searchParams = { episode: "1" };

    pageTitle = `Episode ${searchParams.episode} - ${mediaInfo.title.userPreferred} | Anime Max`;
  }

  return {
    title: mediaInfo ? pageTitle : "Error | Anime Max",
...
export default async function WatchEpisode({
  ...
  async function getVideoStreamingLinkFromSource() {
    ...
    switch (searchParams.source) {
      ...
      case "hianime":
        info = await getHiAnimeStreamingLink({
          episodeId: searchParams.q,
          mediaInfo,
          isDubbed: searchParams.dub == "true",
          isHindi: false,
        ...
        episodeData = info.episodeData as unknown as EpisodeLinksAnimeWatch;
        episodeUrl = info.episodeUrl;
        isEpisodeFromTheSameMedia = info.isEpisodeFromTheSameMedia;
        episodesList = info.episodesList as unknown as EpisodeAnimeWatch[];
        episodeSubtitles = info.episodeSubtitles as unknown as EpisodeLinksAnimeWatch["tracks"];
      ...
      case "toonstream":
        info = await getToonStreamStreamingLink({
          episodeId: searchParams.q,
          mediaInfo,
        });

        episodeData = info.episodeData as unknown as EpisodeLinksGoGoAnime;
        episodeUrl = info.episodeUrl;
        isEpisodeFromTheSameMedia = info.isEpisodeFromTheSameMedia;
        episodesList = info.episodesList as unknown as GogoanimeMediaEpisodes[];
        ...
        <section id={styles.video_container}>
          <VideoPlayer
            ...
            episodeInfo={{
              episodeId: searchParams.q,
              episodeIntro:
                searchParams.source == "aniwatch" || searchParams.source == "hianime" || searchParams.source == "hianime-hindi"
