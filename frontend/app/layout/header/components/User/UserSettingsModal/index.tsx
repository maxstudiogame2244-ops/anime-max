...
function UserSettingsModal({
  ...
  useEffect(() => {
    getUserSavedSettings();
  }, [user, userAnilist]);

  async function getUserSavedSettings() {
    if (!user && !userAnilist) return;

    const userId = user?.$id || `${userAnilist?.id}`;
    const userDoc = await getUserDocument(userId);

    if (!userDoc) return;

    setCurrentLang(userDoc.videoSubtitleLanguage || "English");
    setCurrentSource(userDoc.videoSource || "hianime");
    setCurrentTitleLang(userDoc.mediaTitlePreferredLang || "romaji");
    setCurrentQuality(userDoc.videoQuality || "auto");
    setCurrentShowAdultContent(userDoc.showAdultContent || false);
    setCurrentSkipIntroAndOutro(userDoc.autoSkipIntroAndOutro || false);
    setCurrentNextEpisode(userDoc.autoNextEpisode || false);
    setCurrentPlayWrongMediaEnabled(
      userDoc.playVideoWhenMediaAndVideoIdMismatch || false
    );
  }
  ...
  async function handleUpdateUserInfoForm(
    ...
    if (newImgProfileSelected || form.username.value) {
      if (user) {
        await updateUserProfile(
          form.username.value.slice(0, 40) || user.name
        );
      }
    }
    ...
    await updateUserDocument(userId, {
      ...
      ...(newImgProfileSelected && { photoURL: newImgProfileSelected }),
    });
    ...
    if (onClick) {
      (onClick as (value: void) => void | PromiseLike<void>)();
    }
  }
    ...
    >
      ...
      >
        ...
        <form onSubmit={(e) => handleUpdateUserInfoForm(e)}>
                  ...
                  <>
                    ...
                    >
                      <Image
                        ...
                        src={
                          (user?.prefs?.photoURL as string) ||
                          (anilistUser?.avatar.large as string) ||
                          "/profile_fallback.jpg"
                        }
                        alt={user?.name || (anilistUser?.name as string)}
                        fill
                  ...
                  <React.Fragment>
                    <label>
                      ...
                      <input
                        ...
                        name="username"
                        disabled={
                          !!(user?.prefs?.isAnonymous) || anilistUser?.isUserFromAnilist
                        }
                        defaultValue={user?.name || anilistUser?.name}
                        placeholder={user?.name || anilistUser?.name}
                        required
                      ...
                      <input
                        ...
                        onClick={(e: React.MouseEvent<HTMLInputElement>) =>
                          setNewImgProfileSelected(
                            newImgProfileSelected == (e.target as HTMLInputElement).value
                              ? null
                ...
                <AnimatePresence initial={false} mode="wait">
                  {deleteNotificationsClick && (
                    ...
                    >
                      ...
                      <button onClick={() => deleteOptions("notifications")}>
                        Delete!
                      </button>
                    </motion.div>
                  )}
          ...
          >
            {isLoading ? (
              <LoadingSvg alt="Loading" width={16} height={16} />
            ) : wasSuccessfull ? (
              <>
                <CheckSvg alt="Success" width={16} height={16} /> Saved!
              </>
            ) : (
              "Save Changes"
            )}
