...
import { useAuthState, getUserDocument, type AppwriteUser } from "@/app/lib/appwrite";
...
export default function PlayBtn({
  ...
  async function checkLastEpisodeWatched() {
    ...
    const userPreferredSource = userDoc.videoSource || "aniwatch";

    const episodesWatchedList = JSON.parse(userDoc.episodesWatched || "{}");

    if (episodesWatchedList) setSourceName(userPreferredSource as SourceType["source"]);
    ...
    return fetchMediaEpisodeUrl({ source: userPreferredSource });
  }
  ...
  async function checkIfMediaIsOnKeepWatchingList() {
    ...
    let userKeepWatchingList = JSON.parse(userDoc.keepWatching || "{}");

    const listFromObjectToArray = Object.keys(userKeepWatchingList).map(
      (key) => userKeepWatchingList[key]
    );
  ...
  async function fetchMediaEpisodeUrl({
    ...
    if (hasUser) {
      ...
      const userPreferredSource = userDoc?.videoSource || "gogoanime";
...
function EpisodeNumber({
  ...
  user: AppwriteUser | UserAnilist | null | undefined;
  mediaFormat: string;
