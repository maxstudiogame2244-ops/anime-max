...
export default function EpisodesContainer({
  ...
  async function getUserPreferredSource() {
    ...
    const userSelectedSource =
      userDoc?.videoSource?.toLowerCase() || "crunchyroll";

    fetchEpisodesFromSource(userSelectedSource as SourceType["source"]);

    setCurrEpisodesSource(userSelectedSource as SourceType["source"]);
  }
  ...
  async function getUserEpisodesWatched() {
    ...
    const userEpisodesWatchedList = JSON.parse(userDoc.episodesWatched || "{}");
    ...
    <React.Fragment>
      <div id={styles.episodes_heading}>
        ...
        <EpisodesOptionsPanel
          ...
          userId={user?.$id || anilistUser ? `${anilistUser?.id}` : undefined}
          isAnilistUser={
