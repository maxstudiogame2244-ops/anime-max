...
import { getHiAnimeEpisodeByEpisodeId } from "@/app/api/episodes/hianime/episodesInfo";
import { searchToonStream } from "@/app/api/episodes/toonstream/episodesInfo";
  ...
  async function checkVideoSourceAvailability(
    ...
    videoSourcesAvailable.map(async (source) => {
      switch (source.value) {
        ...
        case "hianime":
          const isHiAnimeAvailable = await getHiAnimeEpisodeByEpisodeId({
            episodeId: "one-piece-100?ep=2142",
            category: "sub",
          }).then((res) => (res?.sources ?? []).length > 0 || false);
          ...
          setSourcesAvailabilty((curr: any) => [
            ...curr,
            {
              name: "HiAnime",
              value: "hianime",
              isAvailable: isHiAnimeAvailable,
            },
          ]);
        ...
        case "hianime-hindi":
          const isHiAnimeHindiAvailable = await getHiAnimeEpisodeByEpisodeId({
            episodeId: "one-piece-100?ep=2142",
            category: "dub",
          }).then((res) => (res?.sources ?? []).length > 0 || false);
          ...
          setSourcesAvailabilty((curr: any) => [
            ...curr,
            {
              name: "HiAnime (Hindi)",
              value: "hianime-hindi",
              isAvailable: isHiAnimeHindiAvailable,
            },
          ]);
        ...
        case "toonstream":
          const isToonStreamAvailable = await searchToonStream({
            query: "naruto",
          ...
          setSourcesAvailabilty((curr: any) => [
            ...curr,
            {
              ...
              value: "toonstream",
              isAvailable: isToonStreamAvailable,
            },
          ]);
