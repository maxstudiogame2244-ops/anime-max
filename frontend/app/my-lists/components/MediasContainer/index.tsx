...
export default function MediasContainer({
  ...
  useEffect(() => {
    if (user?.$id || anilistUser?.id) getUserLists();
  }, [user, anilistUser]);
  ...
  async function getUserLists() {
    ...
    const keepWatchingData = JSON.parse(userDoc.keepWatching || "{}");
    const keepWatchingListFromObjectToArray = Object.keys(keepWatchingData).map(
      (key) => keepWatchingData[key]
    );

    const keepWatchingListFiltered = keepWatchingListFromObjectToArray.filter(
      (item) => item && Object.keys(item).length !== 0
    );

    setKeepWatchingList(keepWatchingListFiltered);

    if (!anilistUser) {
      const mediaListSavedByStatus = JSON.parse(userDoc.mediaListSavedByStatus || "{}");
      const activityLists: { name: string; medias: ListItemOnMediasSaved[] }[] = [];

      Object.keys(mediaListSavedByStatus)?.map((list) =>
        activityLists.push({
          name: list,
          medias: mediaListSavedByStatus[list],
        })
      );
